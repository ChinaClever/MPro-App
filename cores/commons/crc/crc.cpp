/*
 *
 *  Created on: 2022年10月1日
 *      Author: Lzy
 */
#include "crc.h"

ushort Crc::Checksum(const char *ptr, int len)
{
    return qChecksum(ptr, len);
}

ushort Crc::Checksum(const QByteArray &array)
{
    return Checksum(array.data(), array.size());
}

void Crc::ChecksumAppend(QByteArray &array)
{
    ushort crc = Checksum(array);
    array.append(0xff & crc);
    array.append(crc >> 8);
}

#if 0
static ushort calccrc(ushort crc, uchar crcbuf)
{
    uchar x, kkk=0;
    crc = crc^crcbuf;
    for(x=0;x<8;x++)
    {
        kkk = crc&1;
        crc >>= 1;
        crc &= 0x7FFF;
        if(kkk == 1)
            crc = crc^0xa001;
        crc=crc&0xffff;
    }
    return crc;
}
#endif

uint16_t Crc::CRC_16(uint8_t *puchMsg, uint16_t usDataLen)
{
    /* CRC 高位字节值表 */
    static const uint8_t auchCRCHi[] = {
        
    };

    /* CRC 低位字节值表 */
    static const uint8_t auchCRCLo[] = {
        
    };

    uint8_t uchCRCHi = 0xFF;
    uint8_t uchCRCLo = 0xFF;
    unsigned long uIndex;
    while (usDataLen--)
    {
        uIndex = uchCRCHi ^ *puchMsg++;
        uchCRCHi = uchCRCLo ^ auchCRCHi[uIndex];
        uchCRCLo = auchCRCLo[uIndex];
    }

    return (uchCRCHi << 8 | uchCRCLo);

}

ushort Crc::Rtu(uchar *buf, int len)
{
    //ushort crc = 0xffff;
    //for(int i=0; i<len; i++)
    //    crc = calccrc(crc, buf[i]);
    return CRC_16(buf, len);
}

ushort Crc::Rtu(const QByteArray &array)
{
    return Rtu((uchar *)array.data(), array.size());
}

void Crc::RtuAppend(QByteArray &array)
{
    ushort crc = Rtu(array);
    array.append(0xff & crc);
    array.append(crc >> 8);
}

uchar Crc::XorNum(uchar *buf, int len)
{
    uchar xorsum = 0x00;
    for(int i=0; i<len; i++)
        xorsum ^= buf[i];
    return xorsum;
}

uchar Crc::XorNum(const QByteArray &array)
{
    uchar xorsum = 0x00;
    foreach(auto &it, array)
        xorsum ^= it;
    return xorsum;
}

void Crc::XorNumAppend(QByteArray &array)
{
    uchar xorsum = XorNum(array);
    array.append(xorsum);
}




