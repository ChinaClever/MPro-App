...
QByteArray Cascade_Object::readData(uchar fc, uchar addr, ushort size)
{
    ...
    in << fc << addr << size << END_CRC; //cm::CRC16(array);
...
{
    QByteArray array; QDataStream in(&array, QIODevice::WriteOnly);
    in << fc << addr << (ushort)value.size() << value << END_CRC; //cm::CRC16(array);
...
{
    bool ret = false; ushort crc = END_CRC;//cm::CRC16((uchar *)array.data(), array.size()-2);
