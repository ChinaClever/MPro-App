function getColor(e,t,i,o,n)
{
	var a,l,r,u,c,m,f,g,s,d,p,h,v,x,o=o||n.length>0;
	if(n.length>0)
		for(var b=0;b<n.length;b++)
			if(e>n[b].lo&&e<=n[b].hi)
				return n[b].color;
		if(a=i.length,1===a)
			return i[0];
		for(l=o?1/a:1/(a-1),r=[],b=0;b<i.length;b++)
			u=o?l*(b+1):l*b,c=parseInt(cutHex(i[b]).substring(0,2),16),m=parseInt(cutHex(i[b]).substring(2,4),16),f=parseInt(cutHex(i[b]).substring(4,6),16),r[b]={pct:u,color:{r:c,g:m,b:f}};
		if(0===t)
			return"rgb("+[r[0].color.r,r[0].color.g,r[0].color.b].join(",")+")";
		for(var y=0;y<r.length;y++)
			if(t<=r[y].pct)
				return o?"rgb("+[r[y].color.r,r[y].color.g,r[y].color.b].join(",")+")":(g=r[y-1],s=r[y],d=s.pct-g.pct,p=(t-g.pct)/d,h=1-p,v=p,x={r:Math.floor(g.color.r*h+s.color.r*v),g:Math.floor(g.color.g*h+s.color.g*v),b:Math.floor(g.color.b*h+s.color.b*v)},"rgb("+[x.r,x.g,x.b].join(",")+")")
}

function setDy(e,t,i)
{
	(!ie||ie>9)&&e.node.firstChild.attributes.dy&&(e.node.firstChild.attributes.dy.value=0)
}
...
function humanFriendlyNumber(e,t)
{
	var i,o,n,a;
	for(i=Math.pow,o=i(10,t),n=7;n;)
		a=i(10,3*n--),e>=a&&(e=Math.round(e*o/a)/o+"KMGTPE"[n]);
	return e
}
