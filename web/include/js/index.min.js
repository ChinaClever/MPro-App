function buildNav(navArr) {
    for (var html = "", i = 0; i < navArr.length; i++) {
        html += '<li class="nav-item' + (0 === i ? " is-active" : "") + " " + (navArr[i].subNav && navArr[i].subNav.length ? "has-subnav" : "") + '"><a class="nav-item-link" href="' + (navArr[i].url || "javascript:;") + '" target="mainCont">', navArr[i].icon && (html += '<i class="iconfont icon-' + navArr[i].icon + '"></i>'), html += navArr[i].name + "</a>";
